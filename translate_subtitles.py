...
Ferramentas externas necessárias (instale via gerenciador de pacotes):
    - mkvtoolnix  →  fornece: mkvinfo, mkvextract
      Ubuntu/Debian: sudo apt install mkvtoolnix
      macOS:         brew install mkvtoolnix
      Windows:       https://mkvtoolnix.download/

Uso:
    python translate_subtitles.py video.mkv
    python translate_subtitles.py video.mkv --lang eng
    python translate_subtitles.py video.mkv --lang eng --output legenda_pt.srt
    python translate_subtitles.py video.mkv --list-tracks
"""
...
def require_tool(name: str):
    result = run(["which", name], check=False)
...
def convert_to_srt_if_needed(raw_path: str, codec: str) -> str:
    ...
    print("          Instale o ffmpeg ou converta manualmente para SRT e tente novamente.")
    sys.exit(1)
...
def main():
    ...
    parser.add_argument("--source-lang", default="inglês", help="Nome do idioma de origem para o prompt (ex: 'espanhol')")
    parser.add_argument("--output",      default=None,   help="Arquivo de saída .srt (padrão: mesmo nome do mkv)")
    ...
    if not ANTHROPIC_API_KEY:
        ...
        print("  Defina-a antes de executar:")
        print("  export ANTHROPIC_API_KEY='sk-ant-...'")
        sys.exit(1)

    # ── traduzir ─────────────────────────────────────────────────────────────
    print(f"\nTraduzindo para português (em batches de {BATCH_SIZE})…")
