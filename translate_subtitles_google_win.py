...
def main():
    ...
    parser.add_argument("mkv", help="Arquivo .mkv")
    ...
    if not Path(args.mkv).exists():
        print(f"[ERRO] Arquivo não encontrado: {args.mkv}")
        sys.exit(1)
    ...
    out_path = args.output or Path(args.mkv).with_suffix(".pt.srt")
